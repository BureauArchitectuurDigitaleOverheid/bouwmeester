...
// Merge
export async function getMergeCandidates(): Promise<Person[]> {
  return apiGet<Person[]>('/api/auth/merge-candidates');
}

export async function mergePersons(keepId: string, absorbId: string): Promise<Person> {
  return apiPost<Person>(`/api/people/${keepId}/merge/${absorbId}`, {});
}
